/**
 * Peyma Music (app) — Fuerza de contraseña.
 *
 * Copia de `web/src/lib/passwordStrength.ts`. Son dos bundles distintos
 * (Expo y Next) sin paquete compartido entre ellos, igual que pasa con el
 * cliente HTTP y con los textos legales; al tocar uno hay que tocar el
 * otro, porque el medidor tiene que puntuar igual en los dos sitios.
 *
 * Por qué no zxcvbn: mide mucho mejor pero pesa ~800 KB con sus
 * diccionarios, y esto se carga en el alta. Lo que sí importa es no
 * mentir: no se premia "Contraseña1!" por tener mayúscula, número y
 * símbolo — se penalizan los patrones que hacen que una contraseña parezca
 * fuerte sin serlo.
 *
 * Lo único que el backend exige de verdad son 8 caracteres
 * (`registerSchema`). El resto es consejo, no bloqueo.
 */
...
export function evaluatePassword(
  password: string,
  contexto: { email?: string; displayName?: string } = {},
): PasswordStrength {
