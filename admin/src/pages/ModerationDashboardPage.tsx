...
export function ModerationDashboardPage() {
  ...
  return (
    // El título y el subtítulo los pone AdminShell. Repetirlos aquí dejaba la
    // misma frase dos veces, una debajo de la otra, al entrar en la página.
    <AdminShell
      title="Cola de moderación"
      subtitle="Pistas publicadas por creadores, esperando aprobación antes de ser públicas."
      actions={
        tracks.length > 0 ? (
    ...
    >
      {error && (
        <p role="alert" className="mb-6 rounded-xl bg-danger/10 px-4 py-3 text-sm font-semibold text-danger backdrop-blur-xl">
          {error}
        </p>
      )}
