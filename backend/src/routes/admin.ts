...
router.get('/releases/pending', async (_req: AuthRequest, res: Response) => {
  ...
  // Mismo motivo que en /moderation/pending: `genre` es una relación, no una
  // columna, y el panel espera texto plano en cada fila.
  const tracks = rows.map(conGeneroPlano);
